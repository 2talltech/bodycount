...
::tk::Megawidget create ::tk::MiconList ::tk::FocusableWidget {
    variable w canvas sbar accel accelCB fill font index \
	itemList itemsPerColumn list maxIH maxIW maxTH maxTW noScroll \
	numItems oldX oldY options rect selected selection style textList
    constructor args {
	next {*}$args
	set accelCB {}
    }
    ...
    method configure args {
	    ...
	    foreach {opt val} $args {
		    ...
		    -style {
			set style $val
			set fill [ttk::style lookup $style -foreground]
			if {$fill eq ""} { set fill black }
			set bg [ttk::style lookup $style -window]
	...
	set fl [ttk::style lookup $style -foreground]
	if {$fl eq ""} { set fl black }
	$canvas itemconfigure selectionText -fill $fl
	$canvas dtag selectionText
	set cbg [ttk::style lookup $style -selectbackground focus]
	if {$cbg eq ""} { set cbg white }
	set cfg [ttk::style lookup $style -selectforeground]
	if {$cfg eq ""} { set cfg black }
	foreach item $selection {
	    set rTag [lindex $list $item 2]
...
#			-width 400 -height 120 -background white]
	...
	if {[info exists $options(-style)]} {
		set style $options(-style)
	} else {
		set style "."
	}
if 0 { ; # inherit parent's background
	set bg [ttk::style lookup $style -background]
	if {$bg ne ""} {
